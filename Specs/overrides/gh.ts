// gh — GitHub CLI.
//
// Migrated from withfig/autocomplete (src/gh.ts, MIT, last upstream change
// 2025-05) and owned here since 2026-09: upstream stopped publishing, and
// gh kept growing. Edits below the upstream body are marked "Sill:".
  ...
  subcommands: [
    ...
    { name: "api", description: "Make an authenticated GitHub API request" },
    {
      ...
      subcommands: [
        ...
        // Sill: added after upstream stopped (gh 2.40+).
        {
          name: "switch",
    ...
    {
      // Sill: gh's command is `extension` (alias `ext`); upstream had the old
      // plural. Sill shows the longest alias as the row name, so the plural
      // alias is left out on purpose.
      name: ["extension", "ext"],
...
For more information about output formatting flags, see 'gh help formatting'`,
    ...
    // Sill: commands gh added after upstream stopped. Names and one-liners
    // only — their subcommands and options come from `gh <cmd> --help` on each
    // machine (the --help overlay) until someone writes them out here.
    {
      name: "agent-task",
    ...
    {
      name: "copilot",
      description: "Run the GitHub Copilot CLI (preview)",
    },
    {
      name: "discussion",
      description: "Work with GitHub Discussions (preview)",
    },
    {
      name: "label",
    ...
    {
      name: "skill",
      description: "Install and manage agent skills (preview)",
    },
    {
      name: "status",
