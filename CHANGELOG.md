...
- Typing `.` offers `./` on top, so Return runs the line as typed.

### Changed

- The menu bar icon is drawn a little thinner, closer to the weight of the system's own icons.
...
- Fixed folder suggestions not appearing for a path such as `cd ../` after the previous update.
...
- A command recalled from history with the arrow keys or ⌃R opens no popup, so the next arrow keeps moving through history. Typing on that line brings the popup back.
